...
```bash
phone-format-sam$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
phone-format-sam$ sam local start-api
phone-format-sam$ curl http://localhost:3000/
```
...
```bash
phone-format-sam$ sam logs -n HelloWorldFunction --stack-name phone-format-sam --tail
```
...
```bash
phone-format-sam$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name phone-format-sam
```
